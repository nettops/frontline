...
| `npm test` | The gate — determinism, invariants, a 365-day soak, AI behaviour, investigations, war and diplomacy, succession, the briefing, game modes, balance guards, faction beliefs and bonds, memory, failing fronts, the two trades, and a 24-world statistical harness with anomaly detection — run it for the current count rather than trusting a number here, which drifts every session |
| `npm run probe` | The measuring files in `sim/probes/` — the ladder, the floor, the spread, and what a bot finds when it plays. Several minutes, because they simulate thousands of careers to get their numbers |
