...
import { home, memberAge, memberLifeStage, neglectRisk } from './personal';
...
/**
 * Whether this is the boss's own child rather than somebody he hired.
 *
 * Derived, not stored — `CLAUDE.md`'s "reach for a derived read before stored
 * state", and the reason this milestone moves no `SAVE_VERSION`. The only way
 * a household member becomes an `Npc` at all is `gen_family_crossroads`'s
 * `bring_in` branch, which sets `hire.name` to the household member's bare
 * first name. Every other person in the game is generated by `generateNpc`,
 * which always composes `First Last` (or `First "Nickname" Last`) out of two
 * draws. So an exact match against a household name is not a heuristic that
 * usually works: a generated name cannot equal one, because it always has a
 * surname on it.
 *
 * The life-stage gate is the second half and it is not decoration. A child
 * who is still twelve is not in the crew, so a crew member sharing that name
 * is a coincidence and reading him as the boss's son would put a gold tag on
 * a stranger — which is the "everything the player sees is true" rule, not a
 * tidiness preference.
 */
export function isBloodHeir(state: GameState, npc: Npc): boolean {
  return home(state).people.some((p) => {
    if (p.name !== npc.name) return false;
    const age = memberAge(state, p.name, p.relationId);
    if (age === null) return false;
    const stage = memberLifeStage(age).id;
    return stage === 'young_adult' || stage === 'adult';
  });
}
...
/**
 * The half of naming a successor that is about blood rather than seniority.
 *
 * Two branches and no third: either the chair went to the boss's own child,
 * or it went past one. Both are charged where the injury actually lands —
 * with the men on the first, at the kitchen table on the second — and a boss
 * with no grown child in the crew at all passes through here paying nothing,
 * which is most of them.
 *
 * Stacked on top of `NAMING.passedOverGrievance` rather than replacing it,
 * because being passed over and being passed over for the boss's son are two
 * separate things that both happened.
 */
function applyNepotism(state: GameState, npc: Npc): void {
  if (isBloodHeir(state, npc)) {
    for (const capo of activeCapos(state)) {
      if (capo.id === npc.id) continue;
      capo.stats.grievance = clamp(capo.stats.grievance + NEPOTISM.capoGrievance, 0, 100);
      capo.stats.loyalty = clamp(capo.stats.loyalty - NEPOTISM.capoLoyaltyDrop, 0, 100);
      addNote(
        capo,
        state.day,
        'Watched the boss name his own blood successor over a lifetime of service.',
        'bad',
      );
      addLog(state, `${capo.name} views ${npc.name}'s succession as unearned blood nepotism.`, 'crew');
    }
    return;
  }

  // The other door: a grown child who was in the room and did not get it.
  if (!eligibleHeirs(state).some((n) => isBloodHeir(state, n))) return;
  const house = home(state);
  house.neglect = clamp(house.neglect + NEPOTISM.domesticNeglectOnCapoNamed, 0, 100);
  addLog(
    state,
    `Word reached the house that ${npc.name} was named successor over your own blood.`,
    'crew',
  );
}
